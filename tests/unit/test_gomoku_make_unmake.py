...
class TestGomokuMakeUnmake:
    ...
    def test_make_unmake_single_move(self):
        ...
        state = mcts_py.create_gomoku_state(15)

        # Save original state
        original_hash = state.getHash()
        original_player = state.getCurrentPlayer()
        original_string = state.toString()
        original_history_len = len(state.getMoveHistory())
        ...
        assert state.getHash() == original_hash, "Hash mismatch after make/unmake"
        assert state.getCurrentPlayer() == original_player, "Player mismatch after make/unmake"
        assert state.toString() == original_string, "Board state mismatch after make/unmake"
        assert len(state.getMoveHistory()) == original_history_len, "History length mismatch"
    ...
    def test_make_unmake_multiple_moves(self):
        ...
        # Verify restoration
        assert state.getHash() == original_hash, "Hash mismatch after multiple make/unmake"
        assert state.getCurrentPlayer() == 1, "Player should be BLACK after full unwind"
        assert len(state.getMoveHistory()) == 0, "History should be empty"
    ...
    def test_zobrist_hash_consistency(self):
        ...
        assert state.zobrist_hash() == cloned.getHash(), "Zobrist hash mismatch with clone"
    ...
    def test_deep_path_no_drift(self):
        ...
        intermediate_hash = state.getHash()
        intermediate_player = state.getCurrentPlayer()
        ...
        assert state.getHash() == intermediate_hash, "Hash mismatch on re-application"
        assert state.getCurrentPlayer() == intermediate_player, "Player mismatch on re-application"
    ...
    def test_player_flip(self):
        ...
        assert state.getCurrentPlayer() == 2, "Should be White after Black's move"

        # White moves
        undo2 = state.make_move(113)
        assert state.getCurrentPlayer() == 1, "Should be Black after White's move"

        # Unwind
        state.unmake_move(113, undo2)
        assert state.getCurrentPlayer() == 2, "Should be White after unmake"

        state.unmake_move(112, undo1)
        assert state.getCurrentPlayer() == 1, "Should be Black after full unwind"
    ...
    def test_legal_moves_consistency(self):
        ...
        # Get legal moves
        original_moves = set(state.getLegalMoves())
        ...
        restored_moves = set(state.getLegalMoves())
    ...
    def test_terminal_state_handling(self):
        ...
        moves = [112, 97, 113, 98, 114, 99, 115]  # Black has 4 horizontal
        undo_tokens = []
        ...
        # Not terminal yet
        assert not state.isTerminal()

        # Winning move
        winning_move = 116  # Complete 5 in a row
    ...
    def test_board_occupation_correctness(self):
        ...
        assert state.isLegalMove(move)

        # Apply move
        undo = state.make_move(move)

        # Cell should be occupied
        assert not state.isLegalMove(move), "Move should be illegal (occupied)"

        # Reverse move
        state.unmake_move(move, undo)

        # Cell should be empty again
        assert state.isLegalMove(move), "Move should be legal after unmake"
